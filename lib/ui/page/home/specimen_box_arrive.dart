...
import 'package:huayin_logistics/ui/widget/comon_widget.dart' show appBarComon, listTitleDecoration, noDataWidget, showMsgToast;
...
class _SpecimenBoxArrive extends State<SpecimenBoxArrive> {
	...
	Widget build(BuildContext context) {
		...
		return ProviderWidget<SpecimenBoxArriveModel>(
			model: SpecimenBoxArriveModel(context:context,deliveryDriverId:userInfo.id),
			onModelReady:(model){
				model.initData();
			},
			...
			child: new PhysicalModel(
				...
				child: Container(
					...
					decoration: BoxDecoration(
						color:  _activeIndex==index?Color.fromRGBO(56, 219, 254, 0.1):Colors.white,
					),
					child: Container(
									...
									child: new PhysicalModel(
										...
										child: Container(
											...
											child:FlatButton(
												...
												onPressed: (){
													var userInfo = Provider.of<MineModel>(context,listen: false).user?.user;
													model.specimenArriveOperate(item.id,item.updateAt,userInfo.phoneNumber,userInfo.id,userInfo.name,(){
	...
	void specimenArriveInquiryByBoxNo(model,{String boxNo}){
		model.specimenArriveInquiryByBoxNoData(boxNo).then((res){
			if (res!=null) {
				int existIndex=model.list.indexWhere((_element)=>(_element.boxNo==boxNo));//返回第一个满足条件的元素的index  不存在则返回-1
				// print('存在情况');
				// print(existIndex);
				if(existIndex<0){
	...
	Widget _searchTitle(model){
		return Column(
			children: <Widget>[
				...
				Offstage(
					offstage: model.list.length<=0,
					child: listTitleDecoration(title: '标本箱信息',colors: [Color.fromRGBO(91, 168, 252, 1),Color.fromRGBO(56, 111, 252, 1)]),
				)
			],
		);
	}
