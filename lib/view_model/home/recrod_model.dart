...
class RecrodModel extends ViewStateModel {
	...
	Future<bool> recordSavaSubmitData(
		...
		setBusy();
		try {
		await Repository.fetchRecordSavaSubmit(list);
		setIdle();
	...
	//批量录单判断标本箱号是否存在
	Future<List<dynamic>> judgeSpecimenCodeExistData(
		String barCodes
	) async {
		yyDialog = yyProgressDialogBody();
		setBusy();
		try {
			List<dynamic> response=await Repository.fetchJudgeSpecimenCodeExist(barCodes);
			setIdle();
			dialogDismiss(yyDialog);
			return response;
		} catch (e, s) {
